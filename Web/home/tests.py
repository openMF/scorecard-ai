...
class TestPredictionApiV1(TestCase):
    """Integration tests for /api/v1/predict endpoint."""

    @patch("home.views.get_models")
    def test_predict_api_f1_success(self, mock_get_models):
        mock_model = MagicMock()
        mock_model.predict.return_value = np.array([12.5])
        mock_get_models.return_value = {
            "f1_linear": mock_model,
        ...
        self.assertEqual(data["data"]["feature_set"], "F1")
        self.assertEqual(data["data"]["model_name"], "Decision_tree")
        self.assertEqual(data["data"]["prediction_display"], "12.500000")
        self.assertIn("normalized_inputs", data["data"])

    @patch("home.views.get_models")
    def test_predict_api_f2_success_with_confidence(self, mock_get_models):
        mock_model = MagicMock()
        mock_model.predict.return_value = np.array([1])
        ...
        data = response.json()
        self.assertTrue(data["success"])
        self.assertEqual(data["data"]["prediction"], 1)
        self.assertAlmostEqual(data["data"]["confidence"], 0.9, places=6)

    def test_predict_api_invalid_model(self):
        payload = {
            "feature_set": "F1",
            "model_name": "Xgb",
            "inputs": {},
    ...
    def test_predict_api_invalid_inputs_type(self):
        payload = {
            "feature_set": "F1",
            "model_name": "Decision_tree",
            "inputs": [],
        ...
        data = response.json()
        self.assertFalse(data["success"])
        self.assertEqual(data["error"]["code"], "INVALID_INPUTS")
